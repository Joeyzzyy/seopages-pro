            ...
            <div key={index} className={`border rounded-md overflow-hidden bg-white transition-colors ${competitor.logo_fetch_failed ? 'border-amber-300 bg-amber-50/30' : 'border-[#E5E5E5] hover:border-[#D1D5DB]'}`}>
              ...
              {expandedIndex === index && (
                <div className={`px-2 pb-2 pt-1 border-t space-y-1.5 ${competitor.logo_fetch_failed ? 'bg-amber-50/50 border-amber-200' : 'bg-gray-50 border-[#F3F4F6]'}`}>
                  {/* Warning Banner with Mark as OK button */}
                  {competitor.logo_fetch_failed && (
                    <div className="flex items-center justify-between gap-2 px-2 py-1.5 bg-amber-100 border border-amber-200 rounded">
                      <div className="flex items-center gap-2 text-[10px] text-amber-700">
                        ...
                        <span>Website unreachable - please verify manually</span>
                      </div>
                      <button
                        type="button"
                        onClick={() => {
                          const updated = [...competitors];
                          updated[index] = { ...updated[index], logo_fetch_failed: false };
                          updateCompetitors(updated);
                        }}
                        className="px-2 py-0.5 text-[10px] font-medium text-green-700 bg-green-100 hover:bg-green-200 border border-green-300 rounded transition-colors whitespace-nowrap"
                      >
                        ✓ Mark as OK
                      </button>
                    </div>
